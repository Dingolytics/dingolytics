...
import ErrorBoundary from "@redash/vis/components/ErrorBoundary";
 ...
 * @param {string} href
 * 
 * @description Resolve provided link and '/' (root) relative
 * to document's base. If provided href is not related to current
 * document (does not start with resolved root) - return false.
 * Otherwise strip root and return relative url.
 */
export function stripBase(href) {
  const baseHref = trimEnd(url.normalize("/"), "/") + "/";
  href = url.normalize(href);
  if (startsWith(href, baseHref)) {
    return "/" + trimStart(href.substring(baseHref.length), "/");
  }
  return false;
}
